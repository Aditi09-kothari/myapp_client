...
const Movies = () => {
  ...
  const handleChange = async (e) => {
    ...
    await fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=${searchedValue}`
    )
      .then((res) => res.json())
  ...
  const BookmarkMovie = async (movie) => {
    ...
    await fetch(" https://entertainmentapp-backend.onrender.com/api/user/bookmark-movie", {
      method: "POST",
